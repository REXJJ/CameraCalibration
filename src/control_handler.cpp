...
void PCLViewer::updateErrorTable()
{
    ...
    {
        cout<<"Using Planes Instead of object"<<endl;
        string htmlString = "<!DOCTYPE html><htmt><body><style>table, th, td { border: 1px solid black;border-collapse: collapse;}</style><center>Error Metrics</center><table><tr><th>Cloud Id</th><th>Avg Error</th><th>Max Error</th></tr>";
        vector<double> error_avg(clouds_.size(),0.0);
        vector<double> error_max(clouds_.size(),0.0);
        for(int j=0;j<clouds_.size();j++)
        {
            ...
            for(int i=0;i<clouds_[j]->points.size();i++)
            {
                ...
                src[2] = pt.z;
                apply_transformation_optimized(src,out,trans);
                double distance = pointToPlaneDistance(plane_,{out[0],out[1],out[2]});
                average+=distance;
                if(distance>max_error)
                    max_error = distance;
            }
            average=average/clouds_[j]->points.size();
            error_avg[j] = average;
            error_max[j] = max_error;
            cout<<"Average: "<<average<<" Max Error: "<<max_error<<endl;
        }
        for(int j=0;j<error_avg.size();j++)
        {
            double avg_err = int(error_avg[j]*100000.0+0.5)/100.0;
